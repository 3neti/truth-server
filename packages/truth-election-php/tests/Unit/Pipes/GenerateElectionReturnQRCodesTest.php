...
test('generates QR codes for election return and stores them as svg files', function () {
    Storage::fake('local');
    ...
    $pipe = new GenerateElectionReturnQRCodes();

    $result = $pipe->handle($ctx, fn ($ctx) => $ctx);

    expect($result)->toBeInstanceOf(FinalizeErContext::class);

    // Assert at least one QR SVG file was stored
    $files = Storage::disk('local')->files("ER-{$ctx->er->code}/final");

    $qrFiles = array_filter($files, fn ($file) => str_contains($file, 'qr_part_') && str_ends_with($file, '.svg'));

    expect($qrFiles)->not->toBeEmpty();
});
