...
export default function useScannerSession(getArgs: GetDecodeArgs) {
    ...
    function normalizeLine(raw: string): string {
        const out = (raw || '').trim()
        console.log(`${modTag} normalizeLine`, {
            rawLen: raw?.length ?? 0,
            outSample: out.slice(0, 64),
        })
        return out
    }
    ...
    async function decodeNow() {
        console.log(`${modTag} decodeNow() start`, {
            count: lines.value.length,
            preview: lines.value.slice(0, 3),
        })
        const args = getArgs?.() ?? {}
        ...
        try {
            ...
            if (status.value.missing.length) {
                console.warn(`${modTag} missing chunks:`, status.value.missing)
            }
            return res
        } catch (e) {
